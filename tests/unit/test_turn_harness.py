...
async def prepare_database(tmp_path) -> tuple[Database, SlimGuardUser]:
    database = Database(f"sqlite+aiosqlite:///{tmp_path / 'turn-harness.sqlite3'}")
...
def build_runner(
    *,
    ...
    return TurnHarness(
        ...
        compiler=ContextCompiler(manifest=manifest, system_prompt=SYSTEM_PROMPT, tools=registry),
        model=model,
...
async def test_turn_harness_persists_one_complete_primary_path(tmp_path) -> None:
...
async def test_minor_signal_keeps_coaching_tools_and_enters_trusted_context(tmp_path) -> None:
    ...
    try:
        result = await runner.run(
            ...
            grants=TurnGrants(allowed_tool_names=("record_weight",)),
        )
        ...
        assert '"blocks_tools":false' in trusted
        assert '"code":"minor"' in trusted
    finally:
        await database.close()


async def test_turn_harness_uses_same_tool_subset_for_model_and_authorization(tmp_path) -> None:
    database, user = await prepare_database(tmp_path)
    manifest = build_manifest(with_tools=True)
    await AgentVersionRepository(database).register(manifest)
    repository = HarnessStateRepository(database)
    ...
    try:
        result = await runner.run(
            request=initialization_request(user_id=user.id, agent_version_id=manifest.version_id),
            grants=TurnGrants(
...
async def test_context_failure_is_audited_and_stops_before_model_call(tmp_path) -> None:
    database, user = await prepare_database(tmp_path)
    ...
    try:
        ...
        assert [item.item_type for item in items] == [ItemType.USER_MESSAGE, ItemType.ERROR]
