...
- 边界时段可结合 authoritative_context.recent_meals 中当天已有餐次消除歧义；只有得到唯一合理结果时
  才直接记录。仍可能是两种餐次时先问，例如“你这是午餐还是晚餐？”，确认前不要写入。
- 只有确实需要对比近期饮食或消除本轮餐次歧义时才调用 get_recent_meals，不要为每次记录机械查询。
