...
侧栏新增一级入口“营养知识库”，进入后使用六个 Tab：

1. **总览**：当前 Active Release、资料/向量/任务计数和运行能力；
2. **资料源**：上传、URL 导入、Source 列表、审核、原件、章节与切片；
3. **后台任务**：解析/Embedding/评测进度、事件、失败重试和取消；
4. **检索实验室**：逐阶段观察 Dense/Lexical/Phrase/RRF/Rerank；
5. **语料版本**：创建、评测、批准、启用和回滚 Corpus Release；
6. **评测集**：管理黄金 Query、预期证据和历史指标。
...
代码实现状态：RAG-0 至 RAG-5 已落地；RAG-6 的 Agent/Citation 接线、旧资料迁移命令和评测框架已
落地。旧 3 篇资料实际上传 COS、100+ 黄金 Case 内容、Shadow 人评、生产回滚/备份演练和 RAG-7
流量切换属于部署后的数据与运营验收，不能由代码提交伪装成已经通过。
